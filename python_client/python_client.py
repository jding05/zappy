...
while not OVER:
    # if x == 5:
    #     pic = pygame.transform.scale(pygame.image.load('./play.png'), (300, 200))
    #     screen.blit(pic, (100,50)) # 100, 50 -> is the starty point (x, y) from the top left (0, 0)
    # else:
    #     x += 1
# '''
...
# '''
    if x == 1:
        readable, writable, exceptional = select.select([0, s], [1], [], 0)
        for i in readable:
            if i == s:
                data = s.recv(BUFFER_SIZE)
                if not data:

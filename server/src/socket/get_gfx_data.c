...
{
	GET_MAP_DATA_VARS;
	if (NULL == (map = ft_strnew(g_env.map_x * g_env.map_y * 8 + 1)))
		return (NULL);
	i = -1;
	while (++i < g_env.map_y)
	{
		...
		while (++j < g_env.map_x)
		{
			rv = ft_itoa(g_env.map[i][j][0] * pow(10, 6) +
			g_env.map[i][j][1] * pow(10, 5) +
			g_env.map[i][j][2] * pow(10, 4) +
			g_env.map[i][j][3] * pow(10, 3) +
			g_env.map[i][j][4] * pow(10, 2) +
			g_env.map[i][j][5] * pow(10, 1) +
			g_env.map[i][j][6]);
			strcat(map, rv);
			strcat(map, ",");
			free(rv);

...
** y and x, and direction here is the one of other player's y, x, direction position
** to calculate: 
**      calculate the y difference by y1 - y0, x difference by x1 - x0
**      diagolnal with slope -1 get the value by condition [ y1 - y0 = x1 - x0 ] 
**      diagolnal with slope  1 get the value by condition [ y1 - y0 + (x1 - x0) = 0] 
...
**   later, we go through every player to check if the player is the current player
**   if no -> 1. we calculate the 4 possible paths to find the shortest destination
...
void    broadcast(int y, int x, int fd, char *msg)
{
    ...
    while (++i < FD_SETSIZE)
    {
        if (i != fd)
        {
            ...
            nb_dir = calc_direction(pos[j], g_players[i].y, g_players[i].x, g_players.direction);
